...
class Productos extends Conectar
{
    public function get_producto()
    {
        ...
        foreach ($resultado as $d) {
            $Array[] = [
                ...
                'precio' => (float)$d->precio, 'existencias' => (int)$d->existencias
            ];
        }
        return $Array;
    }
    ...
    public function get_productos_x_id($producto_id)
    {
        ...
        $Array = $resultado ? [
            ...
            'precio' => (float)$resultado->precio, 'existencias' => (int)$resultado->existencias
        ] : [];
        return $Array;
    }
    ...
    public function insert_producto($nombre, $precio, $exitencias)
    {
        $conectar = parent::conexion();
        parent::set_names();
        $sql = "INSERT INTO `producto`(`id`,`nombre`, `precio`, `existencias`) VALUES (NULL,?,?,?);";
        
        $sql = $conectar->prepare($sql);
        $sql->bindValue(1, $nombre, PDO::PARAM_STR);
        $sql->bindValue(2, $precio, PDO::PARAM_STR);
        $sql->bindValue(3, $exitencias, PDO::PARAM_INT);
        
        $sql->execute();
        $resultado['SQL'] = $sql->debugDumpParams();
        $resultado['Resp'] = $sql->fetchAll(PDO::FETCH_ASSOC);
        return $resultado;
    }
